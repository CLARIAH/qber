...
var CSDH_API = window.CSDH_API || "http://localhost:5000";
...
module.exports = {
  ...
  retrieveDataverseDefinition: function(settings) {
    $.get(CSDH_API + "/dataverse/definition", { name: settings.name, id: settings.id, user: settings.user }, function(
        response
      ) {
  ...
  retrieveURLBasedDefinition: function(settings) {
    $.get(CSDH_API + "/web/definition", { name: settings.name, url: settings.url, user: settings.user }, function(
        response
      ) {
  ...
  submitDataset: function(settings) {
    ...
    $.post(
        CSDH_API + "/dataset/submit",
        data = JSON.stringify({ dataset: settings.dataset, user: settings.user }),
        function(response) {
          settings.success(response);
        }
      )
